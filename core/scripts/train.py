#!/usr/bin/env python3
"""Train an RL policy inside the container using Isaac Lab's canonical RSL-RL runner.

This is a thin wrapper around ``/workspace/isaaclab/scripts/reinforcement_learning/rsl_rl/train.py``.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train an RSL-RL policy inside the container.")
    parser.add_argument("--task", type=str, required=True, help="Registered gym task ID.")
    parser.add_argument("--headless", action="store_true", help="Disable GUI viewport.")
    ...
    parser.add_argument("--checkpoint", type=str, help="Checkpoint .pt file to resume training from.")
    parser.add_argument("--video", action="store_true", help="Record periodic MP4 clips during training.")
...
def main() -> None:
    ...
    script = _find_rsl_rl_dir() / "train.py"
    ...
    print(f"[INFO] Training task '{args.task}' via {script}")
