...
class DifferentialDriveAction(ActionTerm):
    ...
    def __init__(self, cfg: DifferentialDriveActionCfg, env: ManagerBasedEnv):
        ...
        self._wheel_ids, _ = self._asset.find_joints([cfg.left_wheel_name, cfg.right_wheel_name])
        if len(self._wheel_ids) != 2:
            raise ValueError(
                f"Could not find both wheel joints ('{cfg.left_wheel_name}', '{cfg.right_wheel_name}') "
                f"on asset '{cfg.asset_name}'. Found joint indices: {self._wheel_ids}"
            )
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        v = actions[:, 0] * self.cfg.max_lin_vel
        w = actions[:, 1] * self.cfg.max_ang_vel
    ...
    def apply_actions(self):
        self._asset.set_joint_velocity_target(target=self._processed_actions, joint_ids=self._wheel_ids)
