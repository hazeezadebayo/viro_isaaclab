...
def joint_pos_ref_error(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # Align q_ref to match the robot's joint count
    if q_ref.shape[-1] > n_joints:
        q_ref = q_ref[:, :n_joints]  # truncate extra joints
    elif q_ref.shape[-1] < n_joints:
        pad = torch.zeros(q_ref.shape[0], n_joints - q_ref.shape[-1], device=q_ref.device)
        q_ref = torch.cat([q_ref, pad], dim=-1)  # pad missing joints with 0
...
def joint_vel_ref_error(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # Align dq_ref to match the robot's joint count
