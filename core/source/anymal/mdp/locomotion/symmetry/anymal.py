    ...
    if obs is not None:
        if isinstance(obs, dict):
            ...
            for key, tensor in obs.items():
                repeats = [4] + [1] * (tensor.ndim - 1)
                obs_aug[key] = tensor.repeat(*repeats)
            ...
            obs_aug = torch.zeros((batch_size * 4, *obs.shape[1:]), device=obs.device, dtype=obs.dtype)
            obs_aug[:batch_size] = obs[:]
            obs_aug[batch_size : 2 * batch_size] = _transform_policy_obs_left_right(env.unwrapped, obs)
            obs_aug[2 * batch_size : 3 * batch_size] = _transform_policy_obs_front_back(env.unwrapped, obs)
            obs_aug[3 * batch_size :] = _transform_policy_obs_front_back(
                env.unwrapped, obs_aug[batch_size : 2 * batch_size]
            )
    else:
        obs_aug = None
...
def _transform_policy_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    if obs.shape[1] >= 235 and "height_scan" in env.observation_manager.active_terms["policy"]:
        obs[:, 48:235] = obs[:, 48:235].reshape(-1, 11, 17).flip(dims=[1]).reshape(-1, 11 * 17)
...
def _transform_policy_obs_front_back(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    # note: this is hard-coded for grid-pattern of ordering "xy" and size (1.6, 1.0)
    if obs.shape[1] >= 235 and "height_scan" in env.observation_manager.active_terms["policy"]:
        obs[:, 48:235] = obs[:, 48:235].reshape(-1, 11, 17).flip(dims=[2]).reshape(-1, 11 * 17)
